...
def create_bot_handlers(bot_id: str, bot_instance: Bot, dp: Dispatcher):
            ...
            if callback.data == "promo_pay":
                ...
                try:
                    ...
                    if post_data.get('is_forwarded'):
                        # Используем copy_message — сохраняет премиум эмодзи
    ...
    async def handle_channel_post(message: types.Message):
        ...
        if "#аукцион" in message.text.lower():
            ...
            config.active_auctions[auction_id] = {
                ...
                'discussion_message_id': None,  # Получим когда Telegram перешлёт пост
                'message_id': message.message_id,
                'current_bidder': None,
                'current_bid': 0,
    ...
    async def handle_forwarded_post(message: types.Message):
        ...
        Telegram автоматически пересылает каждый пост канала в привязанную группу.
    ...
    async def handle_comment_reply(message: types.Message):
        ...
        Викторины: правильный ответ — начисление награды.
        Аукционы:
        - ставлю/ставка + число → принятие ставки (если баланс есть и повышение >= 5)
        ...
        # Определяем ID оригинального поста в канале
        ...
        # Проверяем "пас" или "лив" — откат к предыдущей ставке
        ...
        # Проверяем ставку (ставлю N или ставка N)
        ...
        # Проверка: ставка должна быть строго больше предыдущей
        ...
        # Проверка: минимальное повышение
        ...
        # Определяем отображаемое имя (username в приоритете)
...
def create_connection_handlers(bot_instance: Bot, dp: Dispatcher):
    ...
    async def connect_select_modules(callback: types.CallbackQuery, state: FSMContext):
        ...
        if module_type == "takes":
            # Только тейки — просим канал для тейков
            req.modules = ["takes"]
            config.save()
            await callback.message.edit_text(
                "📝 Выбраны тейки\n\n"
                "Отправьте ссылку или ID канала для тейков\n"
                "(если канал закрытый — отправьте числовой ID, например: -1001234567890):"
            )
            await state.set_state(ConnectBotStates.WaitingTakesChannel)

        elif module_type == "shop":
            # Только магазин — просим канал для объявлений
            req.modules = ["shop"]
            config.save()
            await callback.message.edit_text(
                "🛒 Выбран магазин\n\n"
            ...
            # Всё — сначала просим канал для тейков
            req.modules = ["takes", "shop"]
            config.save()
            await callback.message.edit_text(
                "📝🛒 Выбраны тейки + магазин\n\n"
                "Сначала отправьте ссылку или ID канала для ТЕЙКОВ\n"
                "(если канал закрытый — отправьте числовой ID, например: -1001234567890):"
            )
            await state.set_state(ConnectBotStates.WaitingTakesChannel)
    ...
    async def connect_takes_channel(message: types.Message, state: FSMContext):
        ...
        # Числовой ID оставляем как есть, ссылку конвертируем
        ...
        # Если выбраны оба модуля — просим канал для объявлений
        if "shop" in req.modules:
            await message.answer(
                f"✅ Канал для тейков сохранён: {channel}\n\n"
                f"Теперь отправьте ссылку или ID канала для ОБЪЯВЛЕНИЙ\n"
                f"(если канал закрытый — отправьте числовой ID, например: -1001234567890):"
            )
            await state.set_state(ConnectBotStates.WaitingAnnouncementChannel)
        else:
    ...
    async def connect_announcement_channel(message: types.Message, state: FSMContext):
        ...
        req.shop_channel = channel  # shop_channel = канал объявлений
        config.save()
...
async def main():
    ...
    # Проверяем и выводим канал объявлений для отладки
