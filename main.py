...
class Database:
    ...
    def save_published_take(self, user_id: int, bot_id: str, channel_msg_ids: List[int], 
                           chat_id: str, content_type: str, file_ids: List[str] = None, 
                           caption: str = None):
        """Сохранить опубликованный тейк для возможности редактирования."""
        try:
            self.supabase.table('published_takes').insert({
                'user_id': user_id,
                'bot_id': bot_id,
                'channel_message_ids': channel_msg_ids,  # ← ИЗМЕНЕНО: массив ID
                'chat_id': chat_id,
            ...
            logger.info(f"✅ Сохранён тейк {channel_msg_ids} для редактирования")
        except Exception as e:
            logger.error(f"❌ Ошибка сохранения тейка: {e}")

    def get_user_recent_takes(self, user_id: int, bot_id: str, hours: int = 24) -> List[Dict]:
        """Получить последние тейки пользователя за указанный период."""
        try:
            cutoff = datetime.now() - timedelta(hours=hours)
            response = self.supabase.table('published_takes').select('*').eq('user_id', user_id).eq('bot_id', bot_id).gt('published_at', cutoff.isoformat()).order('published_at', desc=True).execute()
            return response.data
        except Exception as e:
            logger.error(f"❌ Ошибка получения тейков: {e}")
            return []

    def delete_published_take(self, take_id: int):
        """Удалить тейк из БД."""
        try:
            self.supabase.table('published_takes').delete().eq('id', take_id).execute()
            logger.info(f"🗑️ Удалён тейк {take_id} из БД")
        except Exception as e:
            logger.error(f"❌ Ошибка удаления тейка: {e}")

    def cleanup_old_takes(self, hours: int = 48):
        """Очистить старые тейки из БД."""
        try:
            cutoff = (datetime.now() - timedelta(hours=hours)).isoformat()
            result = self.supabase.table('published_takes').delete().lt('published_at', cutoff).execute()
            deleted_count = len(result.data) if result.data else 0
            logger.info(f"🗑️ Удалено {deleted_count} старых тейков")
            return deleted_count
        except Exception as e:
            logger.error(f"❌ Ошибка очистки тейков: {e}")
            return 0
...
def create_bot_handlers(bot_id: str, bot_instance: Bot, dp: Dispatcher):
        ...
        if announcement_mods:
            ...
            for mod_uid in announcement_mods:
                try:
                    ...
                    for idx, msg in enumerate(messages):
                        ...
                        has_spoiler = getattr(msg, 'has_media_spoiler', False)
                        if msg.photo:
                            mod_media_group.append(InputMediaPhoto(media=msg.photo[-1].file_id, caption=text_caption, caption_entities=entities, has_spoiler=has_spoiler))
                        elif msg.video:
                            mod_media_group.append(InputMediaVideo(media=msg.video.file_id, caption=text_caption, caption_entities=entities, has_spoiler=has_spoiler))
            ...
            try:
                ...
                for idx, msg in enumerate(messages):
                    ...
                    has_spoiler = getattr(msg, 'has_media_spoiler', False)
                    if msg.photo:
                        media_group.append(InputMediaPhoto(media=msg.photo[-1].file_id, caption=text, caption_entities=entities, has_spoiler=has_spoiler))
                    elif msg.video:
    ...
    async def process_announcement(message: types.Message, state: FSMContext):
        """Обработка одиночного объявления."""
        register_user(message.from_user, bot_id)
        cfg = config.bots.get(bot_id)
        if not cfg or not cfg.announcement_channel:
        ...
        if check_announcement_blocked(message.from_user.id, bot_id):
            await message.answer("🚫 Вы заблокированы для отправки объявлений.", reply_markup=build_main_menu(bot_id))
            await state.clear()
            return

        all_users = db.get_all_users_for_bot(bot_id)
        announcement_mods = [u['user_id'] for u in all_users if (db.get_bot_data(u['user_id'], bot_id).get('is_announcement_mod') or check_admin(u['user_id'], bot_id))]

        if announcement_mods:
            ann_id = str(uuid.uuid4())[:8]
            ann_key = f"ann_{ann_id}"
            config.pending_takes[ann_key] = {
                'user_id': message.from_user.id, 'bot_id': bot_id,
            ...
            if ann_data.get('type') == 'announcement_media_group':
                ...
                for idx, media_info in enumerate(ann_data['media_group']):
                    ...
                    if media_info.get('photo'):
                        media_group.append(InputMediaPhoto(media=media_info['photo'], caption=text, caption_entities=entities, has_spoiler=has_spoiler))
