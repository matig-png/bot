...
class Database:
    ...
    def set_balance(self, user_id: int, bot_id: str, balance: float):
        ...
        val = 0 if is_inf else float(balance)
        try:
            ...
            if existing.data:
                self.supabase.table('balances').update(data).eq('user_id', user_id).eq('bot_id', bot_id).execute()
            else:
    ...
    def get_bot_data(self, user_id: int, bot_id: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'activated_at': None, 'last_promo_at': None,
    ...
    def set_bot_data(self, user_id: int, bot_id: str, **kwargs):
        ...
        try:
            existing_response = self.supabase.table('user_bot_data').select('*').eq('user_id', user_id).eq('bot_id', bot_id).execute()
    ...
    def add_take_timestamp(self, user_id: int, bot_id: str):
        ...
        try:
            ...
            if len(all_records.data) > 20:
                ...
                ids_to_delete = [r['id'] for r in all_records.data[20:]]
                for del_id in ids_to_delete:
                    self.supabase.table('take_timestamps').delete().eq('id', del_id).execute()
        except Exception as e:
    ...
    def find_user_by_input(self, input_str: str) -> Optional[int]:
        ...
        try:
            # Ищем по username
            response = self.supabase.table('users').select('user_id').ilike('username', f'%{input_str}%').execute()
            if response.data:
                return response.data[0]['user_id']
            
            # Ищем по имени
            response = self.supabase.table('users').select('user_id').ilike('name', f'%{input_str}%').execute()
            if response.data:
                return response.data[0]['user_id']
        except Exception as e:
